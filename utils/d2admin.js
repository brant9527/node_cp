...
export default {
  state: {
    ...
    themeList,
    // 现在激活的主题
    themeActiveName: themeList[3].name, // 这应该是一个名字 不是对象
    // 可以在多页 tab 模式下显示的页面
    pagePool: [],
    // 当前显示的多页面列表
    pageOpenedList: [
  ...
  actions: {
    ...
    getPermission({
      ...
      return new Promise(async resolve => {
        Vue.prototype.$axios.get('auth/user/queryUserMenuAndOpList', {
          params: {
            moduleType: '2'
          }
        }).then(res => {
          let templist = res.routeList

          // let routers = filterList(setting.routerPermission, templist)
          // commit('setRouterPermission', routers)
          // 绕过动态路由注入，采取判断是否有权限
          commit('setRoutesAry', templist)
          commit('setBtnPermission', res.menuOpCodeList)
          commit('setMenuPermission', res.menuList)
          commit('setFlag', true)
          return resolve()
        })
      })
  ...
  mutations: {
     ...
     */
    d2adminUtilDb2VuexByUuid(state, {
      key,
      defaultValue
    }) {
      const row = db.get(key).find({
    ...
    d2adminUtilDatabaseUser(state, fn) {
      ...
      const database = db.get('database').find({
        uuid
      })
    ...
    d2adminPageOpenNew(state, {
      ...
      if (pageOpend) {
        ...
        this.commit('d2adminPageOpenedListUpdateItem', {
        ...
        if (tag) {
          this.commit('d2adminTagIncreate', {
            tag,
            params,
            query
          })
     ...
     */
    d2adminPageOpenedListUpdateItem(state, {
      index,
      params,
      query
    }) {
    ...
    d2adminThemeSet(state, themeName) {
      ...
      if (theme) {
        ...
        // 设置为列表第一个主题
        state.themeActiveName = state.themeList[3].name
      }
      // 将 vuex 中的主题应用到 dom
    ...
    d2adminThemeLoad(state) {
      this.commit('d2adminUtilDb2VuexByUuid', {
        key: 'themeActiveName',
        defaultValue: state.themeList[3].name
      })
      this.commit('d2adminTheme2dom')
    },
    /**
    ...
    setRouterPermission(state, list) {
      // store 赋值
      state.routerPermission = list
    },
    setMenuPermission(state, list) {
      // store 赋值
      let menus = getMenuList(list)
      state.menuAside = fliterMenu(menuListDefault, menus)
    },
    setFlag(state, flag) {
      // store 赋值
      state.isAddFlag = flag
    },

    setRoutesAry(state, list) {
      let results = list.map(item => item.menuCode)
...
/**
 * 
 * @param {*} levelPermission 
 * @param {*} list 
 */
function getMenuList(levelPermission) {
  let results = []
  let idx = 0
  for (const key in levelPermission) {
    if (levelPermission.hasOwnProperty(key)) {
      const element = levelPermission[key];
      results.push(element)
      if (element.sonMenuVOList) {
        let result_c = getMenuList(element.sonMenuVOList)
...
function fliterMenu(levelPermission, list) {
  let results = []
  let idx = 0
  console.log(levelPermission)
  levelPermission.forEach((item, index) => {
    let crtItems = list.filter(itemchild => itemchild.menuCode === item.name)
    if (crtItems.length !== 1) return false

    let tmpItem = Object.assign({}, crtItems[0], item)

    results.push(tmpItem)
    if (item.children && item.children.length > 0) {
      let childList = fliterMenu(item.children, list)
      results[idx].children = []
      results[idx].children.push(...childList)
    }
    idx++
  })
  return results
